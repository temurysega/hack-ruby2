module Routing
  class Constraints
    ...
    def amin(prv, op, _now)
      ...
      rej('amount_below_minimum', "сумма ниже min #{fmt(op.amt)} < #{fmt(lim)} (limit_amount_min)")
    end
    def amax(prv, op, _now)
      lim= prv.num('limit_amount_max')
      return nil if lim.nil?||op.amt<=lim
      rej('amount_exceeds_limit', "сумма выше max #{fmt(op.amt)} > #{fmt(lim)} (limit_amount_max)")
    end
    def dyly(prv, op, _now)
      ...
      rej('daily_limit_exceeded', "дневной оборот все #{fmt(cur)} > #{fmt(lim)} (daily_amount_limit)")
    end
    def ipct(prv, _op, _now)
      ...
      return nil if cur<=lim
      rej('in_progress_count_exceeded', "слишком много заявок #{fmt(cur)} > #{fmt(lim)} (in_progress_count_limit)")
    end
    ...
    def ipam(prv, op, _now)
      ...
      return nil if cur<=lim
      rej('in_progress_amount_exceeded', "сумма в работе превышена #{fmt(cur)} > #{fmt(lim)} (in_progress_amount_limit)")
    end
    def reqs(prv, _op, _now)
      return nil if prv['available_requisites']>0
      rej('no_available_requisites', "нет свободных реквизитов #{prv['available_requisites']} (available_requisites)")
    end
    def marg(prv, _op, _now)
      ...
      rej('negative_margin', "маржа провайдера выше мерчанта #{fmt(own)} > #{fmt(mer)} (provider_margin_pct)")
    end
    def bnks(prv, op, _now)
      lst = prv['banks']||[]
      return nil if lst.empty?
      has = lst.include?(op.bank)
      return rej('bank_excluded', "банк в исключ #{op.bank} из #{lst.join(', ')} (exclude_banks)") if prv['exclude_banks']&&has
      return nil if prv['exclude_banks']||has
      rej('bank_not_in_list', "банк не обслуживается #{op.bank}, разрешены #{lst.join(', ')} (banks)")
    end
    def rate(prv, _op, now)
      ...
      rej('rate_limi', "превышена интенсивност #{cnt} > #{lim} заявок за #{@win.to_i} c (requests_minut)")
    end
  end
end
